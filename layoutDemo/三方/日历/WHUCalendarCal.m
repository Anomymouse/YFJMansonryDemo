...
-(NSDictionary*)calendarMapWith:(NSDate*)date{
    ...
    if(firstDayOfMonth.weekday!=2){
        ...
        for(int i=0;i<weekGap;i++){
            ...
            item.year = [NSString stringWithFormat:@"%ld",firstComponent.year];
            item.month = [NSString stringWithFormat:@"%ld",firstComponent.month];
            [self LunarForSolarYear:item andComponent:firstComponent];
            firstComponent.day++;
            if (self.is_loadOtherDate.length == 0) {
    ...
    for(int i=1;i<=days.length;i++){
        ...
        item.day=i;
        item.year = [NSString stringWithFormat:@"%ld",curComponents.year];
        item.month = [NSString stringWithFormat:@"%ld月",curComponents.month];
...
//    if (([FJShareClass share].setAPPLange == SetLangeToZangwen &&[FJShareClass share].setCalendarLange == SegmentedZangwen)||([FJShareClass share].setAPPLange == SetLangeToChinese &&[FJShareClass share].setCalendarLange == SegmentedZangwen)||[FJShareClass share].allYearSet == SegmentedALLYear  ) {
//    
//        NSString *dateStr = [calendarDay.dateStr stringByReplacingOccurrencesOfString:@"-" withString:@"/"];
//        
////        NSString *TibetanCDBpath = [[NSBundle mainBundle]pathForResource:@"TibetanCDB" ofType:@"sqlite"];
//        NSLog(@"%@",TibetanCDBpath);
//        SqlDataBase *sql = [[SqlDataBase alloc]init];
//        [sql open:TibetanCDBpath];
//        NSString *get_sql_Str = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE Language='1' AND Gongli= '%@'",@"Calendar",dateStr];
//        NSArray *getCalendarArray = [sql get_table:get_sql_Str];
//        [sql close];
//    if (getCalendarArray.count != 0) {
//        
//        NSDictionary *zangwenDic = [getCalendarArray firstObject];
//        calendarDay.Chinese_calendar = zangwenDic[@"Zangli"];
//        calendarDay.holiday = zangwenDic[@"tishi"];
//        return;
//        
//    }
//    NSLog(@"%@",getCalendarArray);
//    }
    ...
    if (solarYear_arr.count != 0) {
        
        calendarDay.Chinese_calendar_Month = solarYear_arr[0];
    }
