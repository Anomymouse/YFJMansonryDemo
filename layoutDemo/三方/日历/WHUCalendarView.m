...
-(void)setupView{
    ...
    curDateLbl.text=@"            ";
    curDateLbl.backgroundColor = [UIColor redColor];
...
-(CGSize)getCollectionCellItemSize{
    ...
    return CGSizeMake(itemWidth, ip4 ? itemWidth*0.6 :itemWidth);
}
...
-(CGFloat)getCollectionContentHeight{
    ...
    NSArray* tempArr=_dataDic[@"dataArr"];
    return size.height*(tempArr.count/7.0);
}
...
-(void)layoutSubviews{
    ...
    if(_weekView.subviews.count==0){
        ...
        for(int i=0;i<weekArr.count;i++){
            ...
            [_weekView addConstraint:[NSLayoutConstraint constraintWithItem:lbl attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:_weekView attribute:NSLayoutAttributeTop multiplier:1.0f constant:0]];
            if(preLbl==nil){
        ...
        _calView.backgroundColor=[UIColor clearColor];
        ...
        NSDictionary* metrics2=@{@"gap":@(WHUCalendarView_Margin_Horizon/1.0f)};
